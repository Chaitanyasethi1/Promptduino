...
const ComponentIcon = ({ type, name, id, pins = [], layer = 'all' }) => {
  ...
  const isBase = layer === 'base' || layer === 'all';
  const isOverlay = layer === 'overlay' || layer === 'all';
  ...
  if (t.includes('button') || t.includes('switch')) {
    ...
    return (
      <g>
        {isBase && <rect width="50" height="50" rx="4" fill="#3f3f46" stroke="#18181b" strokeWidth="2" />}
        {isOverlay && (
          <>
          ...
          <>
            {safePins.slice(0, 10).map((p, i) => (
              <g key={`p1-${i}`}>
                ...
                <text x={78 + i * 12} y={23} textAnchor="middle" fill="#fff" fontSize="6px" fontWeight="bold">{p}</text>
              </g>
            ))}
            {safePins.slice(10, 25).map((p, i) => (
              <g key={`p2-${i}`}>
                <circle cx={78 + i * 12} cy="138" r="2.5" fill="#fbbf24" stroke="#fff" strokeWidth="0.5" />
                <text x={78 + i * 12} y={130} textAnchor="middle" fill="#fff" fontSize="6px" fontWeight="bold">{p}</text>
              </g>
            ))}
          </>
        )}
    ...
    <g>
      {isBase && (
        <>
          ...
          <text x={width/2} y={15} textAnchor="middle" fill="#71717a" fontSize="10px" fontWeight="black" fontFamily="monospace">{(name || type || '').toUpperCase()}</text>
        </>
      )}
      {isOverlay && (
        <>
          {safePins.map((p, i) => {
            ...
            return (
              <g key={`f-${i}`}>
                <rect x={px-1} y={90} width="2" height="10" fill="#fbbf24" opacity="0.4" />
                <circle cx={px} cy={100} r="3" fill="#f59e0b" stroke="#fff" strokeWidth="0.5" />
                <text x={px} y={85} textAnchor="end" transform={`rotate(90, ${px}, 85)`} fill="#a1a1aa" fontSize="8px" fontWeight="bold">{p}</text>
              </g>
            );
          })}
        </>
      ...
      <div className="w-full h-full bg-[#18181b] overflow-hidden rounded-lg border border-[#27272a] relative">
        <svg className="w-full h-full" viewBox="0 0 850 600">
          ...
          {partsWithPos.map((p, i) => (
            <motion.g key={`base-${p.id}-${i}`} transform={`translate(${p.x || 0}, ${p.y || 0})`} initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              <ComponentIcon type={p.type} name={p.name} id={p.id} pins={p.pins} layer="base" />
            </motion.g>
          ))}
          ...
          {/* Layer 3: Pins + Labels (Always on Top) */}
          {partsWithPos.map((p, i) => (
            <motion.g key={`overlay-${p.id}-${i}`} transform={`translate(${p.x || 0}, ${p.y || 0})`} initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              <ComponentIcon type={p.type} name={p.name} id={p.id} pins={p.pins} layer="overlay" />
              {/* Overlay Solder Blobs for Wires connected to this part */}
              {diagram.connections && diagram.connections.filter(c => c.from.startsWith(p.id + ':') || c.to.startsWith(p.id + ':')).map((c, ci) => {
                 const isFrom = c.from.startsWith(p.id + ':');
                 const pinName = isFrom ? c.from.split(':')[1] : c.to.split(':')[1];
                 const pos = getPinPos(p.id, pinName);
                 const color = c.color === 'auto' ? '#3b82f6' : (c.color || '#3b82f6');
                 return <circle key={`blob-${ci}`} cx={pos.x - p.x} cy={pos.y - p.y} r="3" fill={color} stroke="#fff" strokeWidth="0.5" />;
              })}
            </motion.g>
          ))}
        </svg>
