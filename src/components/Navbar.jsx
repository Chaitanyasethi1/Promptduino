import { Save, Check, ArrowRight, Loader2 } from 'lucide-react';
    ...
    <div className="flex items-center justify-between h-full px-4 text-sm tracking-wide">
      ...
      <div className="flex items-center space-x-3">
        ...
        <button 
          onClick={uploadCode}
          disabled={isProcessing}
          className="flex items-center justify-center w-8 h-8 text-[#7A7870] hover:text-[#798866] hover:bg-[#E0DCD1] rounded-full transition-colors disabled:opacity-50 mr-2"
          title="Upload"
        >
          {isSimulating ? <Loader2 size={16} className="animate-spin text-[#798866]" /> : <ArrowRight size={18} strokeWidth={2.5} />}
        </button>
        
        <div className="w-px h-5 bg-[#E0DCD1] mx-2"></div>
